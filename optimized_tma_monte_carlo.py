#!/usr/bin/env python3
"""
Optimized TMA Overlay Strategy Monte Carlo Testing

This script tests the TMA Overlay strategy with optimized parameters
using Monte Carlo validation on synthetic data.
"""
...
def run_monte_carlo_validation(trades, num_simulations=MC_SIMULATIONS, confidence_level=CONFIDENCE_LEVEL):
    ...
    output_path = f"reports/optimized_tma_monte_carlo_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    
    with open(output_path, 'w') as f:
        f.write("=== TMA Overlay Strategy Monte Carlo Validation Results (Optimized Parameters) ===\n\n")
...
def main():
    """Run the simplified Monte Carlo validation on TMA strategy with optimized parameters"""
    # Register TMA strategy with our factory
    factory = register_tma_strategy()
    
    # Less restrictive parameters to generate more trades
    optimized_params = {
        'tma_period': 14,
        'atr_multiplier': 1.2,     # Reduced from 1.8 to make bands tighter
        'fast_period': 5,          # Reduced from 8 for more crossovers
        'slow_period': 20,         # Reduced from 21 for more crossovers
        'stop_loss_pct': 0.02,     # Increased from 0.015 for more room
        'take_profit_pct': 0.03,   # Reduced from 0.04 to take profits faster
        'use_tma_direction_filter': False,  # Disabled for more signals
        'use_engulfing_filter': False,      # Disabled for more signals
        'use_ema_confirmation': True,       # Keep EMA confirmation
        'use_band_targets': True,           # Keep dynamic targets
        'use_dynamic_trailing_stop': True   # Keep trailing stops
    }
    
    # Create TMA strategy instance with optimized parameters
    strategy = factory.create_strategy("tma_overlay", custom_config=optimized_params)
    
    logger.info("Testing TMA strategy with less restrictive parameters:")
